...
int main(void)
{
    ...
    // Inicializar estatísticas do cliente
    ClientStats stats = {0}; // Inicializa todos os contadores a zero
    ...
    while(continuar) 
    {
        ...
        if(escolha == 0) 
        {
            ...
            fillSudoku(socketServidor , partialSolution , gameId , logPath, &stats);
            // Escrever estatísticas após cada jogo
            writeClientStats(&stats, logPath, idCliente);
        }
        else if(escolha == 2) {
            continuar = 0;
