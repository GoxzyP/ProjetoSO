...
// -------------------------------------------------------------------------------------------------
// Escreve estatísticas sobrescrevendo o ficheiro (modo "w" em vez de "a")
// -------------------------------------------------------------------------------------------------
void writeStatsOverwrite(const char *filePath, const char *format, ...) { 
    ...
    if (first_write) {
        file = fopen(filePath, "w");  // Primeira escrita: sobrescreve
        first_write = 0;
    } else {
        if (!file) {
            file = fopen(filePath, "a");  // Escreve mais linhas no mesmo ficheiro
        }
    }
    
    if (!file) {
        perror("Error opening/creating stats file");
        return;
    }
    ...
    fflush(file);  // Força escrita imediata no disco
}
